...
double calcXY_J22(double x, double y, double z) {
    double r = hypot(hypot(x, y), z);
    double accelaration = ( (-1/pow(r, 3)) + J2*( (-7.5*pow(z, 2)/pow(r, 7)) + (1.5/pow(r, 5)) ) )*x;
    return accelaration;
} 

double calcZ_J22(double x, double y, double z) {
    double r = hypot(hypot(x, y), z);
...
void symp2_for_J2(Particle* pa, double t) {
    pa->px+=calcXY_J22(pa->qx, pa->qy, pa->qz) *0.5*t;
    pa->py+=calcXY_J22(pa->qy, pa->qx, pa->qz) *0.5*t;
    pa->pz+=calcZ_J22(pa->qz, pa->qx, pa->qy) *0.5*t;
